codeunit 7312 "Create Pick"
{
        ...
        if not IsHandled then begin
            if CurrLocation."Directed Put-away and Pick" then
                QtyBaseMaxAvailToPick := // Total qty (excl. Receive bin content) that are not assigned to any activity/ order
                    ...
                    CalcTotalQtyAssgndOnWhseAct(TempWarehouseActivityLine."Activity Type"::"Put-away", LocationCode, ItemNo, VariantCode) -
            ...
            if TempWhseItemTrackingLine.Find('-') then begin
                repeat
                    if TempWhseItemTrackingLine."Qty. to Handle (Base)" <> 0 then begin
                        ...
                        NewQtyToHandle :=
                          Round(RemQtyToPick / RemQtyToPickBase * TempWhseItemTrackingLine."Qty. to Handle (Base)", UnitOfMeasureManagement.QtyRndPrecision()); // TODO
    ...
    begin
        ...
        if not IsSetCurrentKeyHandled then
            ...
            if CrossDockBin or (CurrLocation."Pick Bin Policy" = "Pick Bin Policy"::"Bin Ranking") then begin
    ...
    begin
        ...
        case WhseSource2 of
            ...
            CreatePickParameters."Whse. Document"::Job:
                LineReservedQty :=
                  WarehouseAvailabilityMgt.CalcLineReservedQtyOnInvt(
                    Database::Job, "Job Planning Line Status"::Order.AsInteger(), CurrJobPlanningLine."Job No.",
    ...
    begin
        ...
        OnCalcQtyOutstandingBaseAfterSetFilters(TempWarehouseActivityLine, TempWhseItemTrackingLine2);
    ...
    begin
        ...
        if CurrLocation."Directed Put-away and Pick" then begin
            if CurrLocation."Adjustment Bin Code" <> '' then
                CalcQtyInWhseEntries.SetFilter(Bin_Code, '<>%1', CurrLocation."Adjustment Bin Code");
