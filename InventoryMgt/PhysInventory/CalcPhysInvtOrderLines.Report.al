report 5880 "Calc. Phys. Invt. Order Lines"
{
    ...
    local procedure CreatePhysInvtOrderLineForItemsWithoutTransactions(ItemNo: Code[20]; VariantCode: Code[10])
    var
        PhysInvtOrderLineArgs: Record "Phys. Invt. Order Line";
    ...
    begin
        ...
        if ItemVariant.FindSet() then begin
            ...
            repeat
                CreatePhysInvtOrderLineForItemsWithoutTransactions(Item."No.", ItemVariant.Code);
            until ItemVariant.Next() = 0
        end;

        OnBeforeCreateNewPhysInvtOrderLineForItemWithNoTransaction(Item, ItemVariantExists);
        if not Item.IsVariantMandatory() or not ItemVariantExists then
            CreatePhysInvtOrderLineForItemsWithoutTransactions(Item."No.", '');
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCreateNewPhysInvtOrderLineForItemWithNoTransaction(var Item: Record Item; var ItemVariantExists: Boolean)
    begin
    end;
