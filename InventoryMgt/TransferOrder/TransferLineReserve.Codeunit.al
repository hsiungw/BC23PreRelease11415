codeunit 99000836 "Transfer Line-Reserve"
{
    ...
    begin
        ...
        if not DerivedTransferLine.IsEmpty() then begin
            ReservationEntry.SetSourceFilter(DATABASE::"Transfer Line", "Transfer Direction"::Inbound.AsInteger(), TransferLine."Document No.", -1, false);
            ReservationEntry.SetSourceFilter('', TransferLine."Line No.");
        end else
            TransferLine.SetReservationFilters(ReservationEntry, "Transfer Direction"::Inbound);
        exit(ReservationEntry.FindLast());
    end;
    ...
    begin
        ...
        if HasErrorOutbnd then begin
            AutoTracking(OldTransferLine, NewTransferLine, ReservationEntry, "Transfer Direction"::Outbound);
            AssignForPlanning(NewTransferLine, "Transfer Direction"::Outbound);
            ...
            then
                AssignForPlanning(OldTransferLine, "Transfer Direction"::Outbound);
        end;

        if HasErrorInbnd then begin
            AutoTracking(OldTransferLine, NewTransferLine, ReservationEntry, "Transfer Direction"::Inbound);
            AssignForPlanning(NewTransferLine, "Transfer Direction"::Inbound);
            if (NewTransferLine."Item No." <> OldTransferLine."Item No.") or
               (NewTransferLine."Variant Code" <> OldTransferLine."Variant Code") or
               (NewTransferLine."Transfer-from Code" <> OldTransferLine."Transfer-from Code")
            then
                AssignForPlanning(OldTransferLine, "Transfer Direction"::Inbound);
        end;
    end;
    ...
    begin
        ...
        with NewTransferLine do begin
            if "Line No." = OldTransferLine."Line No." then
                if "Quantity (Base)" = OldTransferLine."Quantity (Base)" then
                    exit;
            if "Line No." = 0 then
                if not TransferLine.Get("Document No.", "Line No.") then
                    exit;
            for Direction := Direction::Outbound to Direction::Inbound do begin
                ...
                if "Qty. per Unit of Measure" <> OldTransferLine."Qty. per Unit of Measure" then
                    ReservationManagement.ModifyUnitOfMeasure();
                ReservationManagement.DeleteReservEntries(false, "Outstanding Qty. (Base)");
                ReservationManagement.ClearSurplus();
                ReservationManagement.AutoTrack("Outstanding Qty. (Base)");
    ...
    var
        ReservationEntry: Record "Reservation Entry";
    begin
        if FindReservEntry(TransferLine, ReservationEntry, "Transfer Direction"::Outbound) then
            ReservationEntry.ModifyAll("Planning Flexibility", TransferLine."Planning Flexibility");
        if FindReservEntry(TransferLine, ReservationEntry, "Transfer Direction"::Inbound) then
            ReservationEntry.ModifyAll("Planning Flexibility", TransferLine."Planning Flexibility");
    end;
    ...
    begin
        if TransferQty = 0 then
            exit;
        if not FindReservEntry(TransferLine, OldReservationEntry, "Transfer Direction"::Outbound) then
            exit;
    ...
    begin
        ...
        with TransferLine do begin
            ...
            ReservationManagement.SetReservSource(TransferLine, "Transfer Direction"::Inbound);
            if DeleteItemTracking then
                ReservationManagement.SetItemTrackingHandling(1); // Allow Deletion
            ReservationManagement.DeleteReservEntries(true, 0);
    ...
    begin
        ...
        ReservationEntry.SetSourceFilter(DATABASE::"Transfer Line", Direction.AsInteger(), TransferHeader."No.", -1, true);
    ...
    begin
        if not TempTrackingSpecification.FindSet() then
            exit;
        SourceTrackingSpecification.InitFromTransLine(TransferLine, TransferLine."Shipment Date", "Transfer Direction"::Outbound);
    ...
    begin
        ...
        with TempEntrySummary do
            if (TotalQuantity > 0) = Positive then begin
                ...
                "Summary Type" :=
                    CopyStr(StrSubstNo(SummaryTypeTxt, TransferLine.TableCaption(), SelectStr(Direction + 1, Text006Err)), 1, MaxStrLen("Summary Type"));
