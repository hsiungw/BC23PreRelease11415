    ...
    begin
        ...
        with NewPurchaseLine do begin
            if Type <> Type::Item then
                exit;
            if "Document Type" = OldPurchaseLine."Document Type" then
                if "Line No." = OldPurchaseLine."Line No." then
                    if "Quantity (Base)" = OldPurchaseLine."Quantity (Base)" then
                        exit;
            if "Line No." = 0 then
                if not PurchaseLine.Get("Document Type", "Document No.", "Line No.") then
                    exit;
            ReservationManagement.SetReservSource(NewPurchaseLine);
            if "Qty. per Unit of Measure" <> OldPurchaseLine."Qty. per Unit of Measure" then
                ReservationManagement.ModifyUnitOfMeasure();
            if "Outstanding Qty. (Base)" * OldPurchaseLine."Outstanding Qty. (Base)" < 0 then
                ReservationManagement.DeleteReservEntries(true, 0)
            else
