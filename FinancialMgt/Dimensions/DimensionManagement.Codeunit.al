...
codeunit 408 DimensionManagement
{
    ...
    begin
        if DimBuffer.FindSet() then begin
            i := 1;
            repeat
                InsertTempDimBufEntry(TempDimBuf, Enum::TableID::"Dimension Buffer", i, DimBuffer."Dimension Code", DimBuffer."Dimension Value Code");
                i := i + 1;
    ...
    begin
        case Type of
            Type::Resource:
                exit(Enum::TableID::Resource);
            Type::Item:
                exit(Enum::TableID::Item);
            Type::"G/L Account":
                exit(Enum::TableID::"G/L Account");
            else begin
    ...
    begin
        case LineType of
            ...
            4: // "Sales Line Type"::"Fixed Asset"
                exit(Enum::TableID::"Fixed Asset");
            5: // "Sales Line Type"::"Charge (Item)"
                exit(Enum::TableID::"Item Charge");
        end;
    ...
    begin
        ...
        if DefaultDimension."Table ID" = Enum::TableID::Job then
    ...
    begin
        AddDimSource(DefaultDimSource, Enum::TableID::Job, PurchaseLine."Job No.");
        AddDimSource(DefaultDimSource, PurchLineTypeToTableID(PurchaseLine.Type), PurchaseLine."No.");

        OnBeforeGetTableIDsForHigherPrioritiesProcedure(Enum::TableID::"Purchase Line", PurchaseLine, CurrFieldNo, DefaultDimSource);
    ...
    var
        ...
        HighPriorityDefaultDimSource: List of [Dictionary of [Integer, Code[20]]];
    begin
        AddDimSource(DefaultDimSource, Enum::TableID::Job, SalesLine."Job No.");
        AddDimSource(DefaultDimSource, SalesLineTypeToTableID(SalesLine.Type), SalesLine."No.");

        OnBeforeGetTableIDsForHigherPrioritiesProcedure(Enum::TableID::"Sales Line", SalesLine, CurrFieldNo, DefaultDimSource);
        if GetTableIDsForHigherPriorities(
             DefaultDimSource, HighPriorityDefaultDimSource, SourceCode, PriorityTableID)
        then
            DimensionSetID :=
    ...
    var
        ...
        HighPriorityDefaultDimSource: List of [Dictionary of [Integer, Code[20]]];
    begin
        AddDimSource(DefaultDimSource, Enum::TableID::Job, JobJournalLine."Job No.");
        AddDimSource(DefaultDimSource, TypeToTableID2(JobJournalLine.Type.AsInteger()), JobJournalLine."No.");
        AddDimSource(DefaultDimSource, Enum::TableID::"Resource Group", JobJournalLine."Resource Group No.");

        OnBeforeGetTableIDsForHigherPrioritiesProcedure(Enum::TableID::"Job Journal Line", JobJournalLine, CurrFieldNo, DefaultDimSource);
