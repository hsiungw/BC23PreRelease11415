    ...
    begin
        ...
        with NewProdOrderComponent do begin
            if Status = Status::Finished then
                exit;
            if "Line No." = OldProdOrderComponent."Line No." then
                if "Remaining Qty. (Base)" = OldProdOrderComponent."Remaining Qty. (Base)" then
                    exit;
            if "Line No." = 0 then
                if not ProdOrderComponent.Get(Status, "Prod. Order No.", "Prod. Order Line No.", "Line No.") then
                    exit;
            ReservationManagement.SetReservSource(NewProdOrderComponent);
            if "Qty. per Unit of Measure" <> OldProdOrderComponent."Qty. per Unit of Measure" then
                ReservationManagement.ModifyUnitOfMeasure();
            if "Remaining Qty. (Base)" * OldProdOrderComponent."Remaining Qty. (Base)" < 0 then
                ReservationManagement.DeleteReservEntries(true, 0)
            else
