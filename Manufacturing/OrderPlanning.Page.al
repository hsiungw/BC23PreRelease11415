page 5522 "Order Planning"
{
    ...
    begin
        SupplyFromEditable := not (Rec."Replenishment System" in ["Replenishment System"::"Prod. Order",
    ...
    begin
        if Rec."Demand Line No." = 0 then
            case Rec."Demand Type" of
                DATABASE::"Prod. Order Component":
                    Text := Format("Production Order Status".FromInteger(Rec.Status));
                DATABASE::"Sales Line":
                    Text := Format("Sales Document Status".FromInteger(Rec.Status));
                DATABASE::"Service Line":
                    Text := Format("Service Document Status".FromInteger(Rec.Status));
                DATABASE::"Job Planning Line":
    ...
    begin
        case Rec."Demand Type" of
            DATABASE::"Prod. Order Component":
                Text := Format("Production Order Status".FromInteger(Rec.Status));
            DATABASE::"Sales Line":
                Text := Format("Sales Document Type".FromInteger(Rec."Demand Subtype"));
            DATABASE::"Service Line":
                Text := Format("Service Document Type".FromInteger(Rec."Demand Subtype"));
            DATABASE::"Job Planning Line":
                Text := Format("Job Status".FromInteger(Rec.Status));
            DATABASE::"Assembly Line":
                Text := Format("Assembly Document Type".FromInteger(Rec."Demand Subtype"));
        end
    end;
    ...
    begin
        ReplenishmentSystemHideValue := Rec."Replenishment System" = "Replenishment System"::" ";
    end;
