codeunit 1000 "Job Calculate WIP"
{
    ...
    begin
        if (not JobComplete) and (RecognizedCostAmount > UsageTotalCost) and (AccruedCostsJobTask."Job Task No." <> '') then begin
            ...
            InitWIPBufferEntryFromTask(
              AccruedCostsJobTask, AccruedCostsJobWIPTotal, "Job WIP Buffer Type"::"Accrued Costs",
            ...
            if Job."WIP Posting Method" = Job."WIP Posting Method"::"Per Job Ledger Entry" then begin
                InitWIPBufferEntryFromTask(
                  AccruedCostsJobTask, AccruedCostsJobWIPTotal, "Job WIP Buffer Type"::"Applied Costs",
        ...
        with JobTask do
            if not JobComplete then begin
                if "Recognized Costs Amount" <> 0 then begin
                    CreateWIPBufferEntryFromTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Recognized Costs", false);
                    if Job."WIP Posting Method" = Job."WIP Posting Method"::"Per Job" then
                        CreateWIPBufferEntryFromTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Applied Costs", false)
                    else
                        FindJobLedgerEntriesByJobTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Applied Costs");
                end;
                if "Recognized Sales Amount" <> 0 then begin
                    CreateWIPBufferEntryFromTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Recognized Sales", false);
                    ...
                    then
                        CreateWIPBufferEntryFromTask(
                          JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Applied Sales",
                          (("Contract (Invoiced Price)" > "Recognized Sales Amount") and
                           (JobWIPMethod."Recognized Sales" = JobWIPMethod."Recognized Sales"::"Percentage of Completion")))
                    else
                        FindJobLedgerEntriesByJobTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Applied Sales");
                    if "Recognized Sales Amount" > "Contract (Invoiced Price)" then
                        CreateWIPBufferEntryFromTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Accrued Sales", false);
                end;
                ...
                if ("Recognized Sales Amount" = 0) and ("Contract (Invoiced Price)" <> 0) then
                    if Job."WIP Posting Method" = Job."WIP Posting Method"::"Per Job" then
                        CreateWIPBufferEntryFromTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Applied Sales", false)
                    else
                        FindJobLedgerEntriesByJobTask(JobTask, JobWIPTotal, "Job WIP Buffer Type"::"Applied Sales");
            end else begin
                if Job."WIP Posting Method" = Job."WIP Posting Method"::"Per Job Ledger Entry" then begin
    ...
    begin
        ...
        case JobWIPBufferType of
            "Job WIP Buffer Type"::"Applied Costs":
                begin
            ...
            "Job WIP Buffer Type"::"Applied Sales":
                begin
            ...
            "Job WIP Buffer Type"::"Recognized Costs":
                begin
            ...
            "Job WIP Buffer Type"::"Recognized Sales":
                begin
            ...
            "Job WIP Buffer Type"::"Accrued Costs":
                begin
            ...
            "Job WIP Buffer Type"::"Accrued Sales":
                begin
    ...
    begin
        ...
        if JobWIPBufferType = "Job WIP Buffer Type"::"Applied Costs" then
            JobLedgerEntry.SetRange("Entry Type", JobLedgerEntry."Entry Type"::Usage);
        if JobWIPBufferType = "Job WIP Buffer Type"::"Applied Sales" then begin
